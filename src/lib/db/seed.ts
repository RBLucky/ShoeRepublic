...
// Helper function to pick a random number of items from an array
...
async function seed() {
  try {
    ...
    // Seed genders if they don't exist
    const genderRows = [
    ...
    // Seed colors if they don't exist
    const colorRows = [
    ...
    log("Seeding brand: Shoe Republic");
    // Seed the main brand for the store
    ...
    // Fetch all the created lookup data
    ...
    const shoeRepublic = (
      await db.select().from(brands).where(eq(brands.slug, "shoe-republic"))
    )[0];
    const shoesCat = (
    ...
    // Updated product names for "Shoe Republic"
    const productNames = [
      "Urban Strider",
      "Vertex Runner",
      "Apex High-Top",
    ...
    // These images are placeholders and should be replaced with actual product photos
    const sourceImages = [
      "shoe-1.jpg",
      "shoe-2.webp",
      "shoe-3.webp",
    ...
    for (let i = 0; i < productNames.length; i++) {
      ...
      const desc = `Experience unmatched comfort and style with the ${name}.`;
